...
- [ ] 6. **Non-atomic persistence** — `storage.ts` `writeSession()`/`writeIndex()` write directly with `writeFile`. A crash mid-write corrupts `state.json`, which holds the full history including base64 media — an unrecoverable session. Write-to-temp + rename is the standard fix, and matters more here because saves happen multiple times per turn.
