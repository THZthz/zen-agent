...
- Deny-by-default bash sandbox: the rootfs is mounted read-only (`--ro-bind / /`) and only `config.writablePaths` (default `/tmp`, `/var/tmp`; managed with `/writable add|del|clear`, status with no argument) are bind-mounted read-write (`--bind <path> <path>`). An empty list means no writable paths at all.
...
| `writable`     | Manage `config.writablePaths`: `add <path>[,<path>…]` appends paths, `del <path>[,<path>…]` removes them, `clear` empties the list, status (no argument) prints it; persisted in the session row. Each entry is bind-mounted read-write (`--bind <path> <path>`) inside the bash sandbox; the rootfs is read-only by default.                                         |
