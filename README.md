...
An [Agent Client Protocol](https://agentclientprotocol.com) v1 coding agent for [Zed](https://zed.dev) on WSL2/Linux.

## Features

- Unrestricted `bash` tool; no approval prompts
- `read_media` tool to read image/audio for multimodal models
- Live streaming of thinking and answers
- User-defined OpenAI-compatible providers: declare models in `ZEN_AGENT_PROVIDERS` or auto-discover them with `fetchModels: true`
- Persistent sessions in a single SQLite database (`ZEN_AGENT_DB_FILE`) with resume/load across Zed restarts
- Slash commands: `/prompt`, `/sandbox`, `/tools`, one per installed skill ([Agent Skills](https://www.skills.sh/) support)
- Token usage, cache-hit ratio, cost and timing reporting (per-provider currency)
- Optional [bubblewrap](#sandboxing) sandboxing

## Requirements

- Node.js 24+ with npm/npx (`node:sqlite` is built in)
- WSL2/Linux
...
```bash
ZEN_AGENT_PROVIDERS_FILE="$HOME/.config/zen-agent/providers.json" DEEPSEEK_API_KEY=... npx -y zen-agent@latest
# Or
npm run build
ZEN_AGENT_PROVIDERS_FILE="$HOME/.config/zen-agent/providers.json" DEEPSEEK_API_KEY=... node dist/index.js
```
...
```json
{
  "agent_servers": {
    "Zen Agent": {
      ...
      "type": "custom",
      "command": "npx",
      "args": ["-y", "zen-agent@latest"],
      "env": {
        "XDG_DATA_HOME": "/home/<username>/.local/share",
        "ZEN_AGENT_PROVIDERS_FILE": "/home/<username>/.config/zen-agent/providers.json",
        "ZEN_AGENT_DB_FILE": "/home/<username>/.local/share/zen-agent/zen-agent.db",
...
- Set `ZEN_AGENT_DB_FILE` as above: the default location is next to the installed package, i.e. inside npx's cache (`~/.npm/_npx/...`), which npx may clear when it updates the package.
- The `ZEN_AGENT_SANDBOX_BLOCK_SHIM` default resolves inside the installed package, so it needs no override.
...
To run the agent process itself inside bubblewrap, install the package globally (`npm install -g zen-agent`), then point `command` at the bundled wrapper (find the path with `npm root -g`):

```json
"command": "/usr/lib/node_modules/zen-agent/bin/zen-agent-bwrap.sh"
```

and add `"ZEN_AGENT_SANDBOX": "1"` to `env` to sandbox the bash tool as well (see [Sandboxing](#sandboxing)). The wrapper needs a stable path, which a global install provides; npx's cache location is not stable across updates.
