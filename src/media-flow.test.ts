...
function makeCx() {
  ...
  const cx = {
    ...
    notify: vi.fn(async (method: string, params: { sessionId: string; update: acp.SessionUpdate }) => {
...
describe("media prompt flow", () => {
  ...
  it("degrades attached images to placeholder text on text-only models and omits read_media", async () => {
    ...
    expect(
      parts.some(
        (part) =>
          part.type === "text" &&
          part.text!.includes("does not support image input"),
      ),
    ).toBe(true);
  });
  ...
  it("forwards supported images as parts and offers read_media on vision models", async () => {
    ...
    const options = recordedSteps[0]!;
    expect(options.tools?.map((tool) => (tool as typeof READ_MEDIA_TOOL_SCHEMA).function.name)).toEqual([
      "bash",
      "read_media",
    ]);
    const last = options.messages.at(-1) as RecordedMessage;
  ...
  it("read_media failures produce a failed tool result without media injection", async () => {
    ...
    expect(Array.isArray(last.content) === false || (last.content as unknown[]).length > 0).toBe(true);
