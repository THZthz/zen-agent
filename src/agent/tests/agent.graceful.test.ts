...
function statusOf(notifications: NotifyCall[], toolCallId: string): string | undefined {
  const updates = notifications.filter(
    (n) => n.update.sessionUpdate === 'tool_call_update' && n.update.toolCallId === toolCallId,
  );
  const last = updates.at(-1)?.update as
    { sessionUpdate: 'tool_call_update'; status?: string } | undefined;
  return last?.status;
}
...
describe('graceful cancel in runTurn', () => {
  ...
  it('lets the in-flight bash tool finish, persists results, then returns cancelled', async () => {
    ...
    try {
      ...
      expect(ctx.request).not.toHaveBeenCalledWith(
      ...
      const session = (
        agent as unknown as {
          sessions: Map<string, { session: import('../../session/storage.js').StoredSession }>;
        }
      ...
      const usage = (
        agent as unknown as {
          sessions: Map<
            ...
            {
              session: {
                usage: import('../../session/storage.js').SessionUsage;
                turnStats: unknown[];
              };
            }
          >;
  ...
  it('keeps a completed final answer and returns cancelled', async () => {
    ...
    try {
      ...
      expect(ctx.notifications.some((n) => n.update.sessionUpdate === 'agent_message_chunk')).toBe(
        true,
      );
    } finally {
      rmSync(cwd, { recursive: true, force: true });
...
describe('per-turn stats line', () => {
  ...
  it('is shown to the user and persisted in events, but never sent to the LLM', async () => {
    ...
    try {
      ...
      await agent.prompt({ sessionId, prompt: [{ type: 'text', text: 'hi' }] }, cx);
      ...
      const isStatsBubble = (u: acp.SessionUpdate) =>
        ...
        statsRegex.test(String((u as { content: { text?: string } }).content.text ?? ''));

      // Shown to the user live.
      expect(notifications.filter((n) => isStatsBubble(n.update))).not.toHaveLength(0);
