...
function entry(overrides: Partial<Parameters<typeof buildCacheDiagnostic>[0]> = {}): CacheDiagnosticEntry {
...
describe("inferCacheMissReason", () => {
  ...
  it("detects a tool list change", () => {
    ...
    const { reason, detail } = inferCacheMissReason(entry(), prefix({ tools: [TOOL, extra] }), 1_000);
...
describe("appendCacheDiagnostic / latestCacheDiagnostic", () => {
  ...
  it("filters invalid entries on append and skips them when finding the latest", () => {
    ...
    expect(latestCacheDiagnostic([{ garbage: true }, a] as unknown as CacheDiagnosticEntry[])).toEqual(a);
    expect(latestCacheDiagnostic([{ garbage: true }] as unknown as CacheDiagnosticEntry[])).toBeNull();
