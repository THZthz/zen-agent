...
describe("listSkills", () => {
  it("discovers global and project skills and sorts by name", async () => {
    writeSkill(join(homeDir, ".agents", "skills"), "zzz-global", "---\nname: zzz-global\ndescription: Global skill\n---\n");
    writeSkill(join(projectDir, ".agents", "skills"), "aaa-project", "---\nname: aaa-project\ndescription: Project skill\n---\n");
  ...
  it("lets the project skill shadow a same-named global skill", async () => {
    writeSkill(join(homeDir, ".agents", "skills"), "shared", "---\nname: shared\ndescription: Global version\n---\n");
    writeSkill(join(projectDir, ".agents", "skills"), "shared", "---\nname: shared\ndescription: Project version\n---\n");
    const skills = await listSkills(projectDir);
  ...
  it("follows symlinked skill folders (npx skills add default install)", async () => {
    const real = writeSkill(projectDir, "real-skill", "---\nname: symlinked-skill\ndescription: via symlink\n---\n");
...
describe("buildSkillsSection", () => {
  it("renders the catalog with load commands and manual-only instructions", async () => {
    const auto = writeSkill(join(homeDir, ".agents", "skills"), "auto-skill", "---\nname: auto-skill\ndescription: Auto skill\n---\n");
    const manual = writeSkill(join(projectDir, ".agents", "skills"), "manual-skill", "---\nname: manual-skill\ndescription: Manual skill\n---\n");
    const skills = await listSkills(projectDir);
    const section = buildSkillsSection(skills);
    expect(section).toContain("## Skills");
    expect(section).toContain("invoked by hand only");
    expect(section).toContain(`- auto-skill [global] — Auto skill — load with: cat ${join(auto, "SKILL.md")}`);
    expect(section).toContain(`- manual-skill [project] — Manual skill — load with: cat ${join(manual, "SKILL.md")}`);
  });
});
...
describe("buildEnvironmentMessage integration", () => {
  it("includes the skills catalog only with ZEN_AGENT_SHOW_SKILLS_CATALOG=1", async () => {
    writeSkill(join(projectDir, ".agents", "skills"), "frontend-design", "---\nname: frontend-design\ndescription: Build polished frontends.\n---\n");
    ...
    expect(message).toContain("cat " + join(projectDir, ".agents", "skills", "frontend-design", "SKILL.md"));
  });
