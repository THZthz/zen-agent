...
export class ZenAgent {
  ...
  async setSessionConfigOption(
    ...
    if (this.sessionHasStarted(active.session)) {
      throw new Error(`${params.configId} cannot be changed after the first message of a session`);
    }
   ...
   */
  private async settlePreviousTurn(active: ActiveSession | undefined): Promise<void> {
...
const system = buildSystemPrompt(active.session);
  ...
  private async recordCacheDiagnostic(
    ...
    active.session.cacheDiagnostics = appendCacheDiagnostic(active.session.cacheDiagnostics, entry);
    if (
